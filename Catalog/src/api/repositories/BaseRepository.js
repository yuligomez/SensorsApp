export default class BaseRepository {
  ...
  async findOnePopulated(query, relation) {
    try {
      return this.model.findOne(query).populate(relation).exec();
    } catch (error) {
      console.error('[DB]', error);
      return new Error(error);
    }
  ...
  async findPopulated(query, relation, sort) {
    try {
      return this.model.find(query, null, sort).populate(relation).exec();
    } catch (error) {
      console.error('[DB]', error);
      return new Error(error);
    }
  ...
  async findByIdPopulated(id, relation) {
    try {
      return this.model.findById(id).populate(relation);
    } catch (error) {
      console.error('[DB]', error);
      return new Error(error);
    }
